...
async function parseSchedule() {
	...
	for (const content of contents) {
		...
		// Term might still not be updated, include Fall information in that case
		let mostRecentTerm = departmentTitle.indexOf(" Spring");
		if (!mostRecentTerm) {
			mostRecentTerm = departmentTitle.indexOf(" Fall");
		}
		const departmentName = departmentTitle.substring(0, mostRecentTerm);
